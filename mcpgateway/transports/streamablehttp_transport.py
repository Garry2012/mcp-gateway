...
def _check_any_team_for_streamable_rbac(user_context: dict[str, Any] | None) -> bool:
    """Return whether Streamable HTTP RBAC should check across team-scoped roles.

    Both global and server-scoped MCP endpoints must consider team roles for
    sessions and team-scoped API tokens. PermissionService still limits API role
    lookup to token teams; resource visibility and token permission caps are checked
    independently. A public-only API token does not inherit team permissions.
    ...
    if user_context.get("token_use") == "session":
        return True
    return bool(user_context.get("teams"))
...
async def call_tool(
    ...
    if _should_enforce_streamable_rbac(user_context):
        ...
        # Layer 2: RBAC check
        # Sessions and team API tokens need scoped role lookup on either endpoint.
        # Mirrors the decorator's _resolve_team_and_check_mode fallback in rbac.py.
        has_execute_permission = await _check_streamable_permission(
            user_context=user_context,
            permission="tools.execute",
            check_any_team=_check_any_team_for_streamable_rbac(user_context),
        )
            ...
            async with get_db() as check_db:
                ...
                if gateway and getattr(gateway, "gateway_mode", "cache") == "direct_proxy" and settings.mcpgateway_direct_proxy_enabled:
                    ...
                    return await tool_service.invoke_tool_direct(
                        ...
                        server_id=server_id if server_id and server_id != "default_server_id" else None,
                    )
        except Exception as e:
