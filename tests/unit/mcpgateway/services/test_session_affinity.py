...
def test_attach_envelope_trace_context_attaches_context_with_traceparent():
    """Attach the forwarded trace and restore ambient context without leaking into later tests."""
    otel_context = pytest.importorskip("opentelemetry.context", reason="opentelemetry not installed")
    ...
    try:
        assert result is not None
        assert otel_trace.get_current_span().get_span_context().trace_id == int("4bf92f3577b34da6a3ce929d0e0e4736", 16)  # pragma: allowlist secret
    finally:
        sa._detach_envelope_trace_context(result)  # pylint: disable=protected-access
    assert otel_context.get_current() is previous_context
