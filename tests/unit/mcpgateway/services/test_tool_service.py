...
class TestToolService:
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("arguments", [{}, {"param": "private-customer-value"}])
    async def test_invoke_tool_rejects_arguments_failing_input_schema(self, tool_service, mock_tool, mock_global_config_obj, test_db, arguments, monkeypatch, caplog):
        ...
        mock_tool.input_schema = {"type": "object", "properties": {"param": {"type": "integer"}}, "required": ["param"]}
        setup_db_execute_mock(test_db, mock_tool, mock_global_config_obj)
        observations, metrics = MagicMock(), MagicMock()
        observations.start_span.return_value = "rejected-span"
        ...
        monkeypatch.setattr(module, "is_input_capture_enabled", lambda _name: False)
        plugins = MagicMock()
        plugins.invoke_hook = AsyncMock()
        monkeypatch.setattr(tool_service, "_get_plugin_manager", AsyncMock(return_value=plugins))
        trace_token = module.current_trace_id.set("rejected-trace")
        try:
            with pytest.raises(ToolInvocationError, match="input schema validation failed") as exc:
                await tool_service.invoke_tool(test_db, "test_tool", arguments, request_headers=None, server_id="rejected-server")
        finally:
            module.current_trace_id.reset(trace_token)
        assert "private-customer-value" not in str(exc.value)
        assert "private-customer-value" not in caplog.text
        plugins.invoke_hook.assert_not_awaited()
        assert metrics.record_tool_metric.call_args.kwargs["success"] is False
        assert metrics.record_server_metric.call_args.kwargs["server_id"] == "rejected-server"
        assert metrics.record_server_metric.call_args.kwargs["success"] is False
        end = observations.end_span.call_args.kwargs
        assert end["status"] == "error"
        assert end["attributes"]["tool.failure_reason"] == "invalid_arguments"
        assert "private-customer-value" not in str(end)
