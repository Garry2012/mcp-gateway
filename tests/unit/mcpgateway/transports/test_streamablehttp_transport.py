...
@pytest.mark.asyncio
@pytest.mark.parametrize("teams, allowed", [(["team-1"], True), ([], False)])
async def test_global_tool_call_uses_token_scoped_roles(monkeypatch, teams, allowed):
    ...
    permission_check.assert_awaited_once_with(user_context=user, permission="tools.execute", check_any_team=allowed)
...
@pytest.mark.parametrize(
    ...
    [
        (None, False),
        ({"token_use": "session", "teams": []}, True),
        ({"token_use": "api", "teams": ["team-1"]}, True),
        ({"token_use": "api", "teams": []}, False),
        ({"token_use": "api", "teams": None}, False),
        ({"token_use": "api"}, False),
    ],
)
def test_check_any_team_for_streamable_rbac(user_context, expected):
    """Team API tokens use scoped roles on every endpoint; public-only tokens do not."""
    assert tr._check_any_team_for_streamable_rbac(user_context) is expected
