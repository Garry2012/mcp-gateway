...
def test_referenced_local_stylesheets_exist():
    """Templates must not link stylesheets that are not shipped.

    A missing file is served as a JSON 404, which browsers reject with a MIME
    type error on every page load.
    """
    static_dir = Path(__file__).resolve().parents[3] / "mcpgateway" / "static"
    missing = []
    for template in _templates():
        text = template.read_text(encoding="utf-8", errors="replace")
        for href in re.findall(r'<link[^>]+href="\{\{ root_path \}\}/static/([^"]+\.css)"', text):
            if not (static_dir / href).exists():
                missing.append(f"{template.name} -> static/{href}")

    assert not missing, "templates link stylesheets that do not exist: " + "; ".join(missing)
